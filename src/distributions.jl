...
function _mode_rician(ν::T) where {T <: Union{Float32, Float64}}
    ...
    elseif ν < low
        return mode_rician_kernel_small_approx(ν)
    elseif ν < med1
        return ν + mode_rician_kernel_med_1_approx(ν)
    elseif ν < med2
        return ν + mode_rician_kernel_med_2_approx(ν)
    elseif ν < med3
        return ν + mode_rician_kernel_med_3_approx(ν)
    elseif ν < med4
        return ν + mode_rician_kernel_med_4_approx(ν)
    elseif ν < med5
        return ν + mode_rician_kernel_med_5_approx(ν)
    elseif ν < tail
        return muladd(inv(ν), mode_rician_kernel_tail_approx(ν), ν)
    else
        return muladd(inv(ν), mode_rician_kernel_tail_long_approx(ν), ν)
    end
end
...
function _var_mode_rician(ν::T) where {T <: Union{Float32, Float64}}
    ...
    elseif ν < low
        return var_mode_rician_kernel_small_approx(ν)
    elseif ν < med1
        return var_mode_rician_kernel_med_1_approx(ν)
    elseif ν < med2
        return var_mode_rician_kernel_med_2_approx(ν)
    elseif ν < med3
        return var_mode_rician_kernel_med_3_approx(ν)
    elseif ν < med4
        return var_mode_rician_kernel_med_4_approx(ν)
    elseif ν < med5
        return var_mode_rician_kernel_med_5_approx(ν)
    elseif ν < med6
        return var_mode_rician_kernel_med_6_approx(ν)
    elseif ν < tail
        return var_mode_rician_kernel_tail_approx(ν)
    else
        return var_mode_rician_kernel_tail_long_approx(ν)
    end
end
...
mode_rician_kernel_small_approx(x::Float64) = (y = abs2(x); evalpoly(y, mode_rician_kernel_small_coeffs(Float64)))
...
mode_rician_kernel_med_1_approx(x::Float64) = (t = muladd(4.941790729789157, x, -4.478468121693078); evalpoly(t, mode_rician_kernel_med_1_num_coeffs(Float64)) / evalpoly(t, mode_rician_kernel_med_1_den_coeffs(Float64)))
...
mode_rician_kernel_med_2_approx(x::Float64) = (t = muladd(5.442286114641202, x, -7.033317195822817); evalpoly(t, mode_rician_kernel_med_2_num_coeffs(Float64)) / evalpoly(t, mode_rician_kernel_med_2_den_coeffs(Float64)))
...
mode_rician_kernel_med_3_approx(x::Float64) = (t = muladd(4.010555343485661, x, -6.919950279524493); evalpoly(t, mode_rician_kernel_med_3_num_coeffs(Float64)) / evalpoly(t, mode_rician_kernel_med_3_den_coeffs(Float64)))
...
mode_rician_kernel_med_4_approx(x::Float64) = (t = muladd(2.8792452747036545, x, -6.685865787950897); evalpoly(t, mode_rician_kernel_med_4_num_coeffs(Float64)) / evalpoly(t, mode_rician_kernel_med_4_den_coeffs(Float64)))
...
mode_rician_kernel_med_5_approx(x::Float64) = (t = muladd(1.905767999546853, x, -6.087262692129057); evalpoly(t, mode_rician_kernel_med_5_num_coeffs(Float64)) / evalpoly(t, mode_rician_kernel_med_5_den_coeffs(Float64)))
...
mode_rician_kernel_tail_approx(x::Float64) = (y = abs2(3.7188486184122316 / x); evalpoly(y, mode_rician_kernel_tail_coeffs(Float64)))
...
mode_rician_kernel_tail_long_approx(x::Float64) = (y = abs2(50.0 / x); evalpoly(y, mode_rician_kernel_tail_long_coeffs(Float64)))
...
mode_rician_kernel_small_approx(x::Float32) = (y = abs2(x); evalpoly(y, mode_rician_kernel_small_coeffs(Float32)))
...
mode_rician_kernel_med_1_approx(x::Float32) = (t = muladd(6.3156257f0, x, -6.5201635f0); evalpoly(t, mode_rician_kernel_med_1_num_coeffs(Float32)) / evalpoly(t, mode_rician_kernel_med_1_den_coeffs(Float32)))
...
mode_rician_kernel_med_2_approx(x::Float32) = (t = muladd(6.1483426f0, x, -8.320976f0); evalpoly(t, mode_rician_kernel_med_2_num_coeffs(Float32)) / evalpoly(t, mode_rician_kernel_med_2_den_coeffs(Float32)))
...
mode_rician_kernel_med_3_approx(x::Float32) = (t = muladd(6.393863f0, x, -10.69319f0); evalpoly(t, mode_rician_kernel_med_3_num_coeffs(Float32)) / evalpoly(t, mode_rician_kernel_med_3_den_coeffs(Float32)))
...
mode_rician_kernel_med_4_approx(x::Float32) = (t = muladd(4.0923085f0, x, -8.484072f0); evalpoly(t, mode_rician_kernel_med_4_num_coeffs(Float32)) / evalpoly(t, mode_rician_kernel_med_4_den_coeffs(Float32)))
...
mode_rician_kernel_med_5_approx(x::Float32) = (t = muladd(1.3952801f0, x, -4.233612f0); evalpoly(t, mode_rician_kernel_med_5_num_coeffs(Float32)) / evalpoly(t, mode_rician_kernel_med_5_den_coeffs(Float32)))
...
mode_rician_kernel_tail_approx(x::Float32) = (y = abs2(3.75094f0 / x); evalpoly(y, mode_rician_kernel_tail_coeffs(Float32)))
mode_rician_kernel_tail_coeffs(::Type{Float32}) = (0.49999958f0, -0.026648765f0, 0.0015620231f0, -0.0001905378f0)
mode_rician_kernel_tail_long_approx(x::Float32) = (y = abs2(10.0f0 / x); evalpoly(y, mode_rician_kernel_tail_long_coeffs(Float32)))
...
var_mode_rician_kernel_small_approx(x::Float64) = (y = abs2(x); evalpoly(y, var_mode_rician_kernel_small_coeffs(Float64)))
...
var_mode_rician_kernel_med_1_approx(x::Float64) = (t = muladd(5.217004542278532, x, -5.31010224445572); evalpoly(t, var_mode_rician_kernel_med_1_num_coeffs(Float64)) / evalpoly(t, var_mode_rician_kernel_med_1_den_coeffs(Float64)))
...
var_mode_rician_kernel_med_2_approx(x::Float64) = (t = muladd(4.854613121131017, x, -6.871780425599165); evalpoly(t, var_mode_rician_kernel_med_2_num_coeffs(Float64)) / evalpoly(t, var_mode_rician_kernel_med_2_den_coeffs(Float64)))
...
var_mode_rician_kernel_med_3_approx(x::Float64) = (t = muladd(4.62458348298804, x, -8.498786170926266); evalpoly(t, var_mode_rician_kernel_med_3_num_coeffs(Float64)) / evalpoly(t, var_mode_rician_kernel_med_3_den_coeffs(Float64)))
...
var_mode_rician_kernel_med_4_approx(x::Float64) = (t = muladd(3.945692468249597, x, -9.104359925603731); evalpoly(t, var_mode_rician_kernel_med_4_num_coeffs(Float64)) / evalpoly(t, var_mode_rician_kernel_med_4_den_coeffs(Float64)))
...
var_mode_rician_kernel_med_5_approx(x::Float64) = (t = muladd(3.7525997931393746, x, -10.609876915584227); evalpoly(t, var_mode_rician_kernel_med_5_num_coeffs(Float64)) / evalpoly(t, var_mode_rician_kernel_med_5_den_coeffs(Float64)))
...
var_mode_rician_kernel_med_6_approx(x::Float64) = (t = muladd(3.3036845225991565, x, -11.221012841662303); evalpoly(t, var_mode_rician_kernel_med_6_num_coeffs(Float64)) / evalpoly(t, var_mode_rician_kernel_med_6_den_coeffs(Float64)))
...
var_mode_rician_kernel_tail_approx(x::Float64) = (y = abs2(3.699206978772744 / x); evalpoly(y, var_mode_rician_kernel_tail_coeffs(Float64)))
...
var_mode_rician_kernel_tail_long_approx(x::Float64) = (y = abs2(25.0 / x); evalpoly(y, var_mode_rician_kernel_tail_long_coeffs(Float64)))
...
var_mode_rician_kernel_small_approx(x::Float32) = (y = abs2(x); evalpoly(y, var_mode_rician_kernel_small_coeffs(Float32)))
var_mode_rician_kernel_small_coeffs(::Type{Float32}) = (0.4999999f0, 0.25001055f0, 0.06226728f0, -0.013776709f0, -0.029471133f0)
var_mode_rician_kernel_med_1_approx(x::Float32) = (t = muladd(7.2806435f0, x, -6.1525803f0); evalpoly(t, var_mode_rician_kernel_med_1_num_coeffs(Float32)) / evalpoly(t, var_mode_rician_kernel_med_1_den_coeffs(Float32)))
...
var_mode_rician_kernel_med_2_approx(x::Float32) = (t = muladd(5.716995f0, x, -6.6164355f0); evalpoly(t, var_mode_rician_kernel_med_2_num_coeffs(Float32)) / evalpoly(t, var_mode_rician_kernel_med_2_den_coeffs(Float32)))
...
var_mode_rician_kernel_med_3_approx(x::Float32) = (t = muladd(6.9344044f0, x, -10.238322f0); evalpoly(t, var_mode_rician_kernel_med_3_num_coeffs(Float32)) / evalpoly(t, var_mode_rician_kernel_med_3_den_coeffs(Float32)))
...
var_mode_rician_kernel_med_4_approx(x::Float32) = (t = muladd(6.9912963f0, x, -12.330525f0); evalpoly(t, var_mode_rician_kernel_med_4_num_coeffs(Float32)) / evalpoly(t, var_mode_rician_kernel_med_4_den_coeffs(Float32)))
...
var_mode_rician_kernel_med_5_approx(x::Float32) = (t = muladd(6.257741f0, x, -12.931831f0); evalpoly(t, var_mode_rician_kernel_med_5_num_coeffs(Float32)) / evalpoly(t, var_mode_rician_kernel_med_5_den_coeffs(Float32)))
...
var_mode_rician_kernel_med_6_approx(x::Float32) = (t = muladd(2.8811f0, x, -7.4142957f0); evalpoly(t, var_mode_rician_kernel_med_6_num_coeffs(Float32)) / evalpoly(t, var_mode_rician_kernel_med_6_den_coeffs(Float32)))
...
var_mode_rician_kernel_tail_approx(x::Float32) = (y = abs2(2.920515f0 / x); evalpoly(y, var_mode_rician_kernel_tail_coeffs(Float32)))
var_mode_rician_kernel_tail_coeffs(::Type{Float32}) = (1.0000001f0, -0.058620427f0, 0.013730596f0, -0.0025382978f0, 0.00054561417f0)
var_mode_rician_kernel_tail_long_approx(x::Float32) = (y = abs2(12.0f0 / x); evalpoly(y, var_mode_rician_kernel_tail_long_coeffs(Float32)))
